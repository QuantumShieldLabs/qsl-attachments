...
async fn commit_session(
    ...
    read_json(commit).await
}

fn assert_absent(output: &str, needle: &str) {
    assert!(
...
#[tokio::test]
async fn pressure_logs_redact_capability_descriptor_ciphertext_plaintext() {
    let logs = log_capture();
    let fixture = Fixture::new();
    let descriptor_sentinel = "QATT_DESCRIPTOR_SENTINEL_SHOULD_NOT_LOG_NA0283";
    let plaintext_sentinel = "QATT_PLAINTEXT_SENTINEL_SHOULD_NOT_LOG_NA0283";
    let ciphertext_sentinel = b"QATT_CIPHERTEXT_SENTINEL_SHOULD_NOT_LOG_NA0283";
    let wrong_capability_sentinel = "NA0283WRONGCAPABILITYSENTINEL123456";
    ...
    let (parts, request) = one_part_payload(280_502, ciphertext_sentinel, RetentionClass::Short);
    ...
    let upload_reject = fixture
        ...
        .await;
    assert_eq!(upload_reject.status(), StatusCode::PAYLOAD_TOO_LARGE);

    fixture.disk.set_available_bytes(u64::MAX / 4);
    let upload_ok = fixture
        .bytes_request(
            Method::PUT,
            &format!("/v1/attachments/sessions/{session_id}/parts/0"),
            &[("X-QATT-Resume-Token", &resume_token)],
    ...
    let commit_reject = fixture
        ...
        .await;
    assert_eq!(commit_reject.status(), StatusCode::PAYLOAD_TOO_LARGE);

    fixture.disk.set_available_bytes(u64::MAX / 4);
    let commit_body = commit_session(&fixture, &session_id, &resume_token, &request).await;
    ...
    let wrong_fetch = fixture
        .bytes_request(
            ...
            &[("X-QATT-Fetch-Capability", wrong_capability_sentinel)],
            Vec::new(),
        )
        .await;
    ...
    let (_pressure_parts, pressure_request) =
        one_part_payload(280_503, b"pressure-log-trigger", RetentionClass::Standard);
    let pressure_reject = fixture
        ...
        .await;
    assert_eq!(pressure_reject.status(), StatusCode::PAYLOAD_TOO_LARGE);
    ...
    for output in [&log_output, &audit_json] {
        ...
        assert_absent(output, wrong_capability_sentinel);
    }
    assert!(
        log_output.contains("quota_reject") || audit_json.contains("quota_reject"),
        "pressure path should emit redacted quota evidence"
    );
    assert!(
        log_output.contains("object_expired") || audit_json.contains("object_expired"),
        "cleanup path should emit redacted expiry evidence"
    );
}
