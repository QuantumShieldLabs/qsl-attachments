...
- the current durability boundary is one local storage root on one node; graceful same-root restart is in scope, cold full-root backup/restore plus matching service configuration is the only supported backup shape, and abrupt-crash/open-session recovery remains fail-closed plus bounded operator cleanup rather than cross-file transactional durability
